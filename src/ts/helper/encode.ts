...
export function binary_to_decimal(data: Uint8Array): string[] {
  return binary_to_baseX(data, 10, 3);
}

export function binary_to_octal(data: Uint8Array): string[] {
  return binary_to_baseX(data, 8, 3);
}

export function binary_to_hexa(data: Uint8Array): string[] {
  return binary_to_baseX(data, 16, 2);
}

export function binary_to_ascii(data: Uint8Array): string[] {
  return data.reduce((acc, v) => {
    (acc as string[]).push(v < 32 || v > 126 ? '.' : String.fromCharCode(v));
    return acc;
  }, []);
}
...
export function to_binary_array_element(
  data: Uint8Array,
  value: string[]
): HTMLSpanElement[] {
...
export function break_line_array(
  els: HTMLSpanElement[],
  br: number
): HTMLSpanElement[] {
  /**
...
export function create_break_line_field(
  el: HTMLElement,
  lines: number,
  br: number
): void {
